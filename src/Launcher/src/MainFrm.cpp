...
void CMainFrame::CreateFileAndInjectDll(CString& FileName)
{
    ...
    DataPtr pData = std::make_shared<CProcessData>();
...
LRESULT CMainFrame::OnGameNewWnd(WPARAM wParam, LPARAM lParam)
{
    ...
    WndDataPtr pWndData = std::make_shared<CWndData>(*pEmbedInfo);
...
LRESULT CMainFrame::OnGameUpdate(WPARAM wParam, LPARAM lParam)
{
    ...
    for (it = WndToDelete.begin(); it != WndToDelete.end(); ++it)
    {
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    // 创建一个视图以占用框架的工作区
    if (!m_wndView.Create(WS_CHILD | WS_VISIBLE | TCS_FIXEDWIDTH | TCS_FORCELABELLEFT, CRect(0, 0, 0, 0), this, 1))
    {
        TRACE0("未能创建视图窗口\n");
        return -1;
    }
...
void CMainFrame::OnClose()
{
    ...
    {
        if (IDCANCEL == MessageBox(L"关闭本软件将会关闭所有已经打开的程序，确定关闭吗", L"注意", MB_OKCANCEL))
        {
            m_bIsOnClose = FALSE;
            return;
        }
