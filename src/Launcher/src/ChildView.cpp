...
void CChildView::DeleteWnd(int index)
{
    ...
    for (it = m_vecWndData.begin(); it != m_vecWndData.end(); it++)
    {
...
void CChildView::HideAllWnd()
{
    ...
    for (it = m_vecWndData.begin(); it != m_vecWndData.end(); ++it)
    {
        ::SetWindowPos((HWND)(*it)->GetWndData().hWnd, NULL, 0, 0, 0, 0, SWP_ASYNCWINDOWPOS | SWP_HIDEWINDOW | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER);
    }
}
...
void CChildView::UpdateAllWnd()
{
    ...
    for (it = m_vecWndData.begin(); it != m_vecWndData.end(); ++it)
    {
        if (FALSE == IsIconic() && (*it)->GetIndex() == i)
        {
            ...
            ::SetWindowPos((HWND)(*it)->GetWndData().hWnd, HWND_TOP, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, uFlags);
...
BOOL CChildView::IsCovered(HWND hWnd)
{
    ...
    for (; hAboveWnd != NULL; hAboveWnd = ::GetNextWindow(hAboveWnd, GW_HWNDPREV))
    {
...
void CChildView::OnMouseLeave()
{
    ...
    if (m_bIsTabPressed)
    {
        ...
        if (iCurSel >= 0)
        {
            ...
            ::SetWindowPos((HWND)pCur->GetWndData().hWnd, HWND_TOP, pt.x - 50, pt.y - 10, 0, 0, SWP_NOSIZE | SWP_SHOWWINDOW);
